...
namespace Wargon.TinyEcs {
    ...
    public class DependencyContext : IDependencyContext {
        private readonly IDependenciesContainer container;
    ...
    public static class DI {
        ...
        public static IDependencyContext Register<T>() where T : class {
            return GetOrCreateContainer().Register<T>();
        }

        public static IDependencyContext Register<T>(T instance) where T : class {
            return GetOrCreateContainer().Register(instance);
        }

        public static T Get<T>() where T : class {
            return GetOrCreateContainer().Get<T>();
        }

        public static void Build(object instance) {
            GetOrCreateContainer().Build(instance);
        }
    }
}